...
public class StaffController {
    ...
    {
        List<Payment> list = paymentRepository.findAllByStaffId(id);
        for(Payment payment : list)
        {
            paymentRepository.deleteById(payment.getId());
        }

        List<Rental> rentalList = rentalRepository.findAllByStaffId(id);
        for(Rental rental : rentalList)
        {
            System.out.println("Delete rental with id: " + rental.getId());
            rentalRepository.deleteById(rental.getId());
        }
        List<Store> storeList = storeRepository.findAllByManagerStaffId(id);
        for(Store store : storeList)
        {
            System.out.println("Delete store with id: " + store.getId());
            List<Customer> customerList = customerRepository.findAllByStoreId(store.getId());
